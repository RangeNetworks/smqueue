...
SMnet::deliver_msg_datagram(SMqueue::short_msg_pending *smp)
{
	...
	if (s != 0) {
		LOG(ERR) << "deliver_msg_datagram() can't lookup addr/port: " 
		     << host << ":" << port << ", error " << s;
		return false;
	}
	...
	if (ap == NULL) {		// If we walked off bottom of list,
		LOG(ERR) << "Couldn't send datagram to " << host << ":" 
		     << port << " on any socket";
...
SMnet::get_next_dgram (char *buffer, size_t bufferlen, int mstimeout)
{
	...
	for (j = 0; j < numsockets; j++) {	// Walk the sockets.
		...
		{					// input OK
			...
			if (addrlen > sizeof(src_addr)) {
				...
				LOG(ERR) << "recvfrom received address truncated!";
			...
			if (recvlength > bufferlen) {
				...
				LOG(ERR) << "recvfrom data packet truncated, "
				        "buffer has " 
		...
		if (revents & (POLLERR|POLLHUP|POLLNVAL)) {	// errors
			LOG(ERR) << "Poll error " << strerror(errno)
			     << "huh!";
			return -1;
		}
	...
	LOG(ERR) << "Poll() returned " << i << " without any pending I/O";
	return -1;
}
...
SMnet::myhostname()
{
	...
	if (i != 0) {
		LOG(ERR) << "myhostname() can't find our name: error "
		     << gai_strerror(i);
	}
...
SMnet::string_addr (struct sockaddr *sa, socklen_t len, bool withport)
{
	...
	if (len != 0) {
		...
		if (i != 0) {
			LOG(ERR) << "string_addr() can't print IP address: error "
			     << gai_strerror(i);
			if (0 == inet_ntop(sa->sa_family, sa->sa_data, p, plen)) {
				LOG(ERR) << "string_addr() can't print IP address: inet_ntop error "
				     << strerror(errno);
			}
...
SMnet::parse_addr (const char *str, char *sockad, socklen_t maxlen, socklen_t *len)
{
	...
	s = getaddrinfo(host, port, &myhints, &myaddrs);
	if (s != 0) {
		LOG(ERR) << "parse_addr() can't lookup addr/port: " 
		     << host << ":" << port << ", error " << s;
		return false;
	}
...
#define	RAND_DEVICE	"/dev/urandom"
	...
	if (random_fd <= 0) {
		...
		if (random_fd < 0) {
			LOG(ERR) << "Can't open " << RAND_DEVICE;
		}
	}
	...
	do {
		if (random_fd > 0) {
			...
			if (i != sizeof(randy.randbuf)) {
				LOG(ERR) << "Can't read from " << RAND_DEVICE;
